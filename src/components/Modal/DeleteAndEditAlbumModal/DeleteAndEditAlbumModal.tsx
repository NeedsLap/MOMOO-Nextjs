...
export default function DeleteAndEditAlbumModal({
  ...
  const handleEditAlbum = async () => {
    ...
    setAlbums(prev => prev.map(v => (v.id === albumId ? { ...v, name: editAlbumName } : v)));
        ...
        >
          ...
          <div className="modal-list">
            ...
            <button type="button" onClick={handleDeleteAlbum}>
              Delete
              <Image width={20} height={20} src="/icons/delete-red.svg" alt="휴지통 아이콘" />
            </button>
          </div>
          <button onClick={handleEditAlbum} className="edit-btn" type="submit">
            저장
          </button>
          <button type="button" className="close-button" onClick={onClose} aria-label="모달 닫기">
